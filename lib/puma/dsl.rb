...
module Puma
  ...
  class DSL
    ...
    # Allows the app server to respond with Expect: 100-Conitnue header
    # instead of puma when set to true. When set true, if the app responds back 
    # with HTTP status 100 puma will send the request body
    ...
    def custom_100_continue_header_response(which=false)
      @options[:custom_100_continue_header_response] = which
    end
